...
const MeetingRoom = () => {
  ...
  const sendMessage = () => {
    ...
    setMessages((prev) => [
      ...prev,
      { type: "chat", message: input, userName: "You", time: new Date().toLocaleTimeString() },
    ]);
    setInput("");
  };
    ...
    <div className="min-h-screen w-full bg-linear-to-br from-slate-950 via-slate-900 to-slate-800 text-white flex flex-col">
      ...
      <div className="bg-slate-900/70 backdrop-blur-xl border-b border-slate-800 px-4 py-2.5 flex justify-between items-center gap-2 shrink-0 sticky top-0 z-30">
        <div className="min-w-0">
          ...
          <p className="text-slate-400 text-[11px] truncate flex items-center gap-1 leading-tight">
            Code:{" "}
            <span className="text-slate-300 font-mono">{meeting.meetingCode}</span>
          </p>
        </div>
        <div className="flex items-center gap-1.5 relative shrink-0">
          <button
            onClick={() => setShowParticipants((prev) => !prev)}
            className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 px-2.5 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer"
          >
            <Users className="h-3.5 w-3.5" />
            {participants.length || 1} Participant{(participants.length || 1) > 1 ? "s" : ""}
          </button>
          {showParticipants && (
            <div className="absolute top-10 right-0 w-56 max-w-[75vw] bg-slate-900 border border-slate-800 rounded-xl shadow-xl z-20 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-800 font-semibold text-sm">
                Participants ({participants.length || 1})
              </div>
              <div className="max-h-72 overflow-y-auto py-2">
                {(participants.length ? participants : [{ userName, userId }]).map((p, i) => (
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-red-600/90 hover:bg-red-600 px-2.5 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer"
          >
      ...
      <div className="flex flex-col lg:flex-row flex-1 gap-4 p-4">
        ...
        <div className="flex-1 rounded-3xl bg-slate-950/40 flex flex-col">
          ...
          {/* Controls - single row jaisa screenshot mein hai */}
          <div className="bg-slate-900/70 backdrop-blur-xl border-t border-slate-800 py-3 sm:py-5 flex justify-center items-center gap-3 flex-wrap px-3 sm:px-4">
            ...
            <button
              onClick={() => navigate(`/meeting/${id}/summary?transcript=${encodeURIComponent(transcript)}`)}
        ...
        <div className="w-full lg:w-[340px] xl:w-[380px] bg-slate-900/70 backdrop-blur-xl border-t lg:border-t-0 lg:border-l border-slate-800 flex flex-col min-h-[400px] lg:min-h-0 overflow-hidden rounded-2xl lg:rounded-none">
          ...
          {/* Tasks section - screenshot jaisa */}
          <div className="border-t border-slate-800 p-3">
            <div className="flex justify-between items-center mb-2">
              <h3 className="font-semibold text-sm">Tasks</h3>
              <button
                onClick={() => navigate(`/meeting/${id}/tasks`)}
